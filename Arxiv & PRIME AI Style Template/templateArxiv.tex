...
\\ \\
A \textit{Logarithmic Signal-Power Distance} (LSD) was calculated to indicate the efficiency of the filter by taking the original signal $z_n$ and the filtered audio signal $z''_n$ into account:
	\begin{equation}
	LSD=10log_{10} \left(\frac{\overline{(z''_n-\overline{z''_n})^2}}{\overline{(z_n-\overline{z_n})^2}}\right).
	\end{equation}
\\ \\
...
The LSD measure shows that effect in one figure and gives it a name.
...
\hline
LSD &       \multicolumn{2}{c|}{-16.29dB}       &     \multicolumn{2}{c|}{ $7.63\cdot 10^{-5}$dB}      \\
\hline
...
The filter decreases the unwanted behaviors selectively for the frequency bins by attenuation factor based on the T60 decay matrix in numerical plot \ref{fig:Decaymatrix}. 
...
The author gratefully acknowledges Simon Ciba, M.A., known from e.g. "WhisPER – A New Tool for Performing Listening Tests", for his careful review of this work and his valuable comments. \\ \\
