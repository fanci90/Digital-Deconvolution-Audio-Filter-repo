...
The time domain \textit{impulse response}, e.g. from equation \eqref{eq:IR}, is transformed by STFT, to decay certain frequency bins from blind $T60$ estimate. 
...
The filter is designed aggressively on purpose and filters out reverberation and produces a dry sound with a minimum of room and recording setup characteristic, allowing the direct path signal to be dominant.
It might be necessary to weight the output and mix it into the original signal, if it appears too sharp for the listener.
However, as the filter does exactly what it should, the output might need further processing for more pleasant sound characteristic, but is obviously a excellent start for further audio processing.
