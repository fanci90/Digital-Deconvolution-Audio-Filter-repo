...
\author{
Stefan Ciba \\
\href{https://www.researchgate.net/profile/Stefan-Ciba}{https://www.researchgate.net/profile/Stefan-Ciba} \\
  %% examples of more authors
   \And
 % M. Ciba \\
%\href{https://www.researchgate.net/profile/Manuel-Ciba}{https://www.researchgate.net/profile/Manuel-Ciba} \\
  %% \AND
  %% Coauthor \\
  %% Affiliation \\
  %% Address \\
...
%    \midrule
%    A & Input  & $\sim$100     \\
%    B     & Output terminal & $\sim$10      \\
%    C     & Prediction Error       & up to $10^6$  \\
%    \bottomrule
%  \end{tabular}
%  \label{tab:table}
%\end{table}
...
\section{Supplements}
%\subsection{Linear sine sweep}
%The implementation of the linear frequency sweep generation and periodic repetition was done as follows. The core chirp equation follows the standard linear sweep formulation [3][7], while the modulo operation in $$ t_n $$ handles the periodic extension through array tiling.
% 
%### Time Vector
%$$
%t_n = \frac{n}{f_s}, \quad n = 0,1,\ldots,N-1 \quad \text{where } N = T_{\text{dur}} \cdot f_s
%$$
%- $$ f_s $$: Sampling rate (`sample_rate`)
%- $$ T_{\text{dur}} $$: Duration in seconds (`duration_sec`)
%
%### Linear Sine Sweep Signal
%$$
%x(t) = \sin\left(2\pi \left( f_0 t + \frac{f_1 - f_0}{2T_{\text{dur}}} t^2 \right)\right)
%$$
%- $$ f_0 $$: Start frequency (`start_freq`)
%- $$ f_1 $$: End frequency (`end_freq`)
%- Matches `method='linear'` in `scipy.signal.chirp` [3][7]
%
%### Periodic Extension (array tiling):
%$$
%x_{\text{periodic}}(t) = \bigoplus_{k=0}^{P-1} x(t - kT_{\text{dur}})
%$$
%- $$ P $$: Number of periods (`periods`)
%- $$ \oplus $$: Concatenation/tiling operation (`np.tile`)
%**Full equation with discrete indexing:**
%$$
%x_{\text{periodic}}[n] = \sin\left(2\pi \left( f_0 t_n + \frac{f_1 - f_0}{2T_{\text{dur}}} t_n^2 \right)\right), \quad t_n = \frac{n \bmod %N}{f_s}
%$$

%[1] https://tex.stackexchange.com/questions/162648/how-to-draw-a-sinewave-in-tikz
%[2] https://stackoverflow.com/questions/22597392/generating-swept-sine-waves
%[3] https://www.recordingblogs.com/wiki/sine-sweep
%[4] https://tex.stackexchange.com/questions/40160/math-symbol-question-vertical-bar-for-evaluated-at
%[5] https://matthewvaneerde.wordpress.com/2009/08/03/how-to-calculate-a-sine-sweep-the-wrong-way/
%[6] https://www.hsu-hh.de/ant/wp-content/uploads/sites/699/2022/02/Exponential-Sine-Sweep-Measurement-Implementation-Targeting-FPGA-Platforms-KLEMDNOWAKRIVERAGERATZOeLZERKLAUER.pdf
%[7] https://endaq.com/pages/sine-sweep-testing
%[8] https://en.wikibooks.org/wiki/LaTeX/Advanced_Mathematics

%\subsection{}
The Python-code of the studied method and audio samples are to be found at: \href{https://git...}{https://git...}.
%Interessant: Ein System, das ein sich wiederholendes Verhalten zeigen sollte, kann evtl. in das Reservoir "eingelernt" werden. Wenn die Prediction und der Messwert größere Abweichungen zeigt, dann kann eine Benachrichtigung / Warnung gegeben werden, damit sich z.B. jemand das System genauer anschaut.
