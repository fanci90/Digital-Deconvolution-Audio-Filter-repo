...
%% Title
\title{Dereverberation Filter by Deconvolution with Frequency Bin Specific Faded Impulse Response
%%%% Cite as
...
\begin{abstract} 
This work introduces a robust single-channel inverse filter for dereverberation of non-ideal recordings, validated on real audio.
...
\keywords{dereverberation \and spectral impulse response modification \and inverse filtering method}
...
The time domain \textit{impulse response}, e.g. from equation \eqref{eq:IR}, is transformed by STFT, to fade frequency bins based on blind $T60$ estimate. 
...
The most significant estimated pitch of the audio increases, which is due to filtered room characteristic that allowed deeper frequencies to fade slowly.
...
Slow fading, early reflections and some unwanted behavior, that account for late reflections, can be seen in the original unmodified impulse response. 
...
Because the impulse response from the speech record was different from the drum record, filter results differ a lot.
...
%\subsection{Signal Filtering and Processing}
